...
test("baseline branch never loads nancy, but does get the branch-neutral shopping-agent AGENTS.md (no confirmation section)", () => {
  withTempRoot((runsRoot) => {
    ...
    assert.ok(!/task confirmation/i.test(agentsMd), "baseline has no NanCy loaded and no concept of a Task Confirmation at all — a real run once leaked this concept into the shared section by mistake");
...
test("nancy branch loads nancy's plugin dir, writes AGENTS.md with both the shopping-agent framing and the confirmation section, and sets telegram off", () => {
  withTempRoot((runsRoot) => {
    ...
    assert.ok(agentsMd.includes("authorizes the whole outcome"), "nancy branch must get the shopping-specific confirmation-phrasing note, kept separate from the branch-neutral shopping-agent section");
    const identityMd = readFileSync(join(paths.workspaceDir, "IDENTITY.md"), "utf8");
