...
- `docs/architecture/confirmation-forms.md` — **v1 implemented** (`src/confirmation/forms.ts`): NanCy-generated number/range/text/boolean forms plus a response-mode menu appended after gap-detection's note. Purely presentational — only a literal "y" ever grants a task; select/multiselect authorization fields and their trusted-config option-list question remain deferred/undecided.
- `docs/architecture/task-confirmation-menu.md` — **option 3 implemented, presentationally**: a menu of response modes for the confirmation step (best-judgment autonomy, Clarify Mode, the confirmation-forms.md form, free text, or a narrower "gather info first" confirmation) instead of a bare y/n. Key finding held even more strongly than predicted: all four non-Clarify-Mode options need zero NanCy code — only "always confirm when needed" requires Part C's Clarify Mode to actually exist.
