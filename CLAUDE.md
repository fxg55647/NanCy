...
- `docs/architecture/task-confirmation-menu.md` — **design discussion, not implemented**: a menu of response modes for the confirmation step (best-judgment autonomy, Clarify Mode, the confirmation-forms.md checklist, free text, or a narrower "gather info first" confirmation) instead of a bare y/n. Key finding: four of the five options need zero NanCy code changes — they're just how the target agent shapes its own confirmation text/staging; only "always confirm when needed" requires Part C's Clarify Mode to actually exist.
