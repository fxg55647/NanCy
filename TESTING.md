...
- **A hard-coded low output-token cap can silently fail every real review on a "thinking" model.** Found via a real end-to-end run (`tools/mobile-chat-poc/`'s A2A test against a live gateway with NanCy loaded), not something the mocked suite could ever catch: `src/analysis/client.ts`'s `callLlm()` used a flat `maxOutputTokens: 300` for Gemini. On a thinking-capable model (NanCy's own live configured reviewer, `gemini-3.8-flash`, included) internal reasoning tokens are drawn from that *same* budget before any visible reply — 300 left no room for the actual two-line verdict once thinking consumed it, so the call routinely hit `MAX_TOKENS`, `callLlm()` threw, and NanCy's fail-closed design turned that into a denied/failed review with no obvious "budget" symptom. Fixed by raising the shared budget (`ANALYSIS_MAX_OUTPUT_TOKENS`) and, for Gemini specifically, disabling thinking outright (`generationConfig.thinkingConfig.thinkingBudget: 0`) — this reviewer call is meant to be a fast one-shot judgment, not a reasoning task. This is exactly the class of bug Option B exists for: Option A's mocked `fetch` always returns exactly the canned response you told it to, so a real provider's actual token accounting can only ever be caught here.
