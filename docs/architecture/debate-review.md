...
- Every mode, including single-pass `off`, fails closed on full-review exceptions, missing/malformed outputs and timeouts for tools and outbound messages. Final CLARIFY blocks. One final denial is recorded per action; review infrastructure failures are not security-denial signals.
...
At proposal time, tool calls failed closed while outbound-message exceptions failed open. The implementation now cancels the message on every required-review failure in all modes, including single-pass. This prevents a provider outage or malformed response from becoming an outbound bypass, at the cost of suppressing ordinary replies during that outage.
