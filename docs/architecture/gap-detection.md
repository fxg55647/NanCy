...
## Non-goal: gap detection is not the security gate

The confirmation description and destination now pass a separate required security review first. That review blocks on BLOCK, CLARIFY, missing analysis, provider error, or malformed output. Gap detection runs only after that gate has accepted the request, and remains advisory:
...
- any failure in this second advisory call is treated as "no gaps found," not as a reason to hold back a confirmation the security review already accepted; and
...
The rationale: missing task details and unsafe outbound content are different decisions. The security check must fail closed because the wrapper otherwise becomes an exfiltration channel. The optional completeness hint may fail open because its worst outcome is that the human does not get an extra hint.
...
Default `true`. Set to `false` to disable the advisory check and note. The required confirmation security review still runs. There is no separate rate limit or cost cap for the advisory call; confirmations are assumed to be rare.
