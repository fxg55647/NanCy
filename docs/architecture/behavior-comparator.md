...
Status: **v1, one scenario, calibration-run mode only**. This is the design record for `tools/checkpoint-recorder/`, `tools/scenario-shop/`, and `tools/comparator/` — read it before extending any of them, so later changes stay consistent with the decisions below instead of silently drifting from them. None of this is part of NanCy itself; it's a dev-only instrument that runs *alongside* NanCy (or without it) to observe the difference. The full pipeline — a real isolated Gateway process, model registration, plugin trust/hook gating, the A2A channel (see below), a real turn, clean shutdown — is now empirically verified via `tools/comparator/test/driver-smoke.test.ts` (real, but free: a valid model with a deliberately invalid API key, so it fails fast on auth and never reaches a successful generation). See "Known limitations" for what real (billed) calibration runs against this pipeline have found so far, and what's still open.
...
- **A real Gateway-only run (before the A2A switch) surfaced a real agent-behavior gap, separate from the transport bug.** With Gateway/hooks/manifest fixes in place but still on the (since-abandoned) plain-CLI transport, a real run completed with no purchase in either branch: the model never called `search_products`/`buy_product` at all, instead asking real-world questions (retailer, delivery address, payment method) as if this were an open-ended real purchase — the scenario's system-prompt framing likely needs to more strongly signal "use the available tools," and/or the bare test-agent workspace's lack of any identity file caused an unrelated "what should I call you" onboarding digression that burned turns unproductively. Still open with the A2A transport — a fresh real calibration run is the next step, now that message_sending and the reviewer's token budget are both confirmed fixed.
